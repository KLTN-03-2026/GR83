...
import {
  ...
  DRIVER_RIDE_REQUEST_SEEN_NOTIFICATION_IDS_STORAGE_KEY,
  consumeDriverRideRequest,
  enqueueDriverRideRequest,
  findNearbyDriverRideRequest,
  loadDriverRideRequestQueue,
...
function isCustomerTrackedRideStatus(value) {
  ...
  return normalizedStatusToken !== 'hoanthanh' &&
...
function mergeRideBookingSnapshot(currentBooking, nextBooking) {
  if (!nextBooking || typeof nextBooking !== 'object') {
    return currentBooking;
  }

  const currentBookingCode = getRideBookingCode(currentBooking);
...
function normalizeRideSearchLocation(location, fallbackLabel) {
  if (typeof location === 'string') {
    return createLocationRecord(location || fallbackLabel, {
      source: 'search',
    });
  }

  if (!location || typeof location !== 'object') {
    return createLocationRecord(fallbackLabel, {
      source: 'search',
    });
  }

  return createLocationRecord(location.label ?? location.description ?? location.main_text ?? fallbackLabel, {
    position: location.position ?? location,
    source: location.source ?? 'search',
  });
}

function normalizeRideSearchResult(response) {
  ...
  return {
    ...response,
    pickup: normalizeRideSearchLocation(response.pickup, 'Điểm đón'),
    destination: normalizeRideSearchLocation(response.destination, 'Điểm đến'),
    results: Array.isArray(response.results) ? response.results : [],
  };
}

function findTrackedCustomerRideBooking(items, preferredBookingCode = '') {
  const candidateItems = Array.isArray(items) ? items : [];
  const normalizedPreferredBookingCode = String(preferredBookingCode ?? '').trim();

  if (normalizedPreferredBookingCode) {
...
export default function HomePage() {
  ...
  const [activeDriverTripAction, setActiveDriverTripAction] = useState(null);
  ...
  useEffect(() => {
    ...
    const scanForDriverRideRequest = async () => {
      ...
      driverRideRequestScanInFlightRef.current = true;
      ...
      try {
        ...
        try {
          driverPosition = await getCurrentPositionAsync();
        } catch {
          driverPosition = null;
        }

        const match = findNearbyDriverRideRequest(queue, driverPosition, DRIVER_RIDE_REQUEST_NEARBY_DISTANCE_KM);

        if (!match) {
          return;
        }

        setActiveDriverRideRequest({
          request: match.request,
          distanceKm: match.distanceKm,
          isNearby:
            !Number.isFinite(match.distanceKm) || match.distanceKm <= DRIVER_RIDE_REQUEST_NEARBY_DISTANCE_KM,
        });
        setDriverRideRequestModalOpen(true);
      } catch {
        return;
      } finally {
        driverRideRequestScanInFlightRef.current = false;
      }
    };
  ...
  useEffect(() => {
    ...
    const syncCustomerRideState = async (event = null) => {
      if (customerRideSyncInFlightRef.current) {
        return null;
      }

      customerRideSyncInFlightRef.current = true;

      try {
        const eventBooking = event?.booking && typeof event.booking === 'object' ? event.booking : null;
        ...
        return applyCustomerRideBooking(matchedBooking);
      } catch {
        if (event?.booking && typeof event.booking === 'object') {
          return applyCustomerRideBooking(event.booking);
        }

        return null;
      } finally {
        customerRideSyncInFlightRef.current = false;
      }
    };

    const syncDriverRideState = async () => {
      if (driverRideSyncInFlightRef.current) {
        return null;
      }

      driverRideSyncInFlightRef.current = true;
    ...
    const handleRideEvent = (event = {}) => {
      ...
      if (eventId && customerRideEventIdRef.current === eventId) {
        return;
      }

      if (eventId) {
        customerRideEventIdRef.current = eventId;
      }
      ...
      if (isDriverRole) {
          ...
          if (enqueuedRequest && typeof window !== 'undefined' && typeof StorageEvent === 'function') {
            window.dispatchEvent(
              new StorageEvent('storage', {
                key: DRIVER_RIDE_REQUEST_QUEUE_STORAGE_KEY,
                oldValue: null,
                newValue: null,
        ...
        if (eventType === 'ride.trip.status.updated') {
          ...
          setActiveDriverTripAction((currentRequest) => {
            ...
            return {
              ...currentRequest,
              tripStatus: event?.tripStatus ?? currentRequest.tripStatus,
              tripStatusLabel: event?.tripStatusLabel ?? currentRequest.tripStatusLabel,
              tripStatusTone: event?.tripStatusTone ?? currentRequest.tripStatusTone,
    ...
    const disconnectRideEventStream = connectRideEventStream({
      accountId,
      roleCode: normalizedUserRoleCode,
  ...
  const runRideSearch = async (vehicleOverride = activeVehicle) => {
    ...
    try {
      ...
      const normalizedResponse = normalizeRideSearchResult(response);

      setSearchResult(normalizedResponse);
      setSelectedRideId(normalizedResponse?.results?.[0]?.id ?? null);
    } catch (error) {
  ...
  const handlePrimaryBookingAction = async () => {
    ...
    const rideIdFromPreview = searchResultItems.some((item) => item.id === previewSelectedRideId)
  ...
  const handleCancelBooking = async (cancelDetails = null) => {
    const cancelReason =
      typeof cancelDetails === 'string'
        ? cancelDetails.trim()
        : String(cancelDetails?.reasonText ?? cancelDetails?.reasonLabel ?? '').trim();

    const currentBooking = bookingSuccess;
    const bookingCode = String(currentBooking?.bookingCode ?? '').trim();

    if (!bookingCode) {
      showMiniToast('Không thể xác định chuyến cần hủy.', 'error', 2200);
      return null;
    }

    try {
      const tripStatusResult = await rideService.updateTripStatus(
        bookingCode,
        'DaHuy',
        {
          ...
          cancelReasonCustomReason: String(cancelDetails?.customReason ?? '').trim(),
        },
      );

      if (!tripStatusResult?.success) {
        throw new Error(tripStatusResult?.message || 'Không thể hủy chuyến trên máy chủ.');
      }
  ...
  setBookingRatingModalOpen(false);
      ...
      setBookingSelectedPromoId('');
      setPreviewModalOpen(false);
      showMiniToast(cancelReason ? `Đã hủy chuyến. Lý do: ${cancelReason}` : 'Đã hủy chuyến.', 'success');

      return tripStatusResult;
    } catch (error) {
      showMiniToast(
        error?.message || 'Không thể hủy chuyến trên máy chủ. Vui lòng thử lại.',
        'error',
        2400,
      );
  ...
  const openDriverRideRejectModal = (request) => {
    if (!request) {
      return;
    }

    setDriverRideRejectModalOpen(true);
  };

  const closeDriverRideRejectModal = () => {
    setDriverRideRejectModalOpen(false);
  };

  const closeDriverTripActionModal = () => {
    setDriverTripActionModalOpen(false);
    setDriverTripActionBubbleOpen(false);
    setActiveDriverTripAction(null);
  };

  const minimizeDriverTripActionModal = () => {
    if (!activeDriverTripAction) {
      setDriverTripActionBubbleOpen(false);
      setDriverTripActionModalOpen(false);
      return;
    }
    ...
    setDriverTripActionModalOpen(false);
    setDriverTripActionBubbleOpen(true);
  };

  const restoreDriverTripActionModal = () => {
    if (!activeDriverTripAction) {
      setDriverTripActionBubbleOpen(false);
      return;
    }
  ...
  const cleanupDriverRideRequestInBackground = (request) => {
  ...
  const handleDriverRideRequestAccept = async (request) => {
    let tripStatusResult;
    const driverAccountId = String(authenticatedUser?.id ?? '').trim();

    try {
      tripStatusResult = await rideService.updateTripStatus(
        request.bookingCode,
        DRIVER_TRIP_STAGE_TO_SERVER_STATUS.accepted,
        { driverAccountId },
      );
    } catch (error) {
    ...
    closeDriverRideRequestModal();
    setActiveDriverTripAction({
      ...request,
      driverAccountId,
      tripStatus: tripStatusResult.tripStatus,
      tripStatusLabel: tripStatusResult.tripStatusLabel,
      tripStatusTone: tripStatusResult.tripStatusTone,
      updatedAt: tripStatusResult.updatedAt,
    });
    setDriverTripActionBubbleOpen(false);
    setDriverTripActionModalOpen(true);
  ...
  const handleDriverTripActionAdvance = async (request, nextStage) => {
    const serverStatus = DRIVER_TRIP_STAGE_TO_SERVER_STATUS[nextStage];
    const driverAccountId = String(request.driverAccountId ?? authenticatedUser?.id ?? '').trim();

    if (!serverStatus) {
      throw new Error('Trạng thái chuyến không hợp lệ.');
    }

    const tripStatusResult = await rideService.updateTripStatus(request.bookingCode, serverStatus, {
      driverAccountId,
    });

    if (!tripStatusResult?.success) {
      throw new Error('Không thể cập nhật trạng thái chuyến lên máy chủ.');
    }

    setActiveDriverTripAction((currentRequest) => {
      if (!currentRequest || currentRequest.bookingCode !== request.bookingCode) {
        return currentRequest;
      }
  ...
  const handleDriverTripActionCancel = async (request, cancelDetails = null) => {
    const driverAccountId = String(request.driverAccountId ?? authenticatedUser?.id ?? '').trim();
    const cancelReason =
      typeof cancelDetails === 'string'
        ? cancelDetails.trim()
    ...
    if (!tripStatusResult?.success) {
      throw new Error('Không thể hủy chuyến trên máy chủ.');
    }

    closeDriverTripActionModal();
    showMiniToast(cancelReason ? `Đã hủy chuyến. Lý do: ${cancelReason}` : 'Đã hủy chuyến.', 'success', 1800);
  };
  ...
  return (
    <div className="page-shell">
      <Header
        ...
        accountPhone={authenticatedUser?.phone ?? ''}
      ...
      <main>
                ...
                <div className="booking-results">
                  <div className="booking-results__summary">
                    <div>
                      ...
                      <strong>
                        {searchPickupLabel} → {searchDestinationLabel}
                      </strong>
                    </div>
        ...
        {bookingTrackingBubbleOpen && bookingSuccess && !bookingTrackingModalOpen
          ? createPortal(
              <button
                ...
                type="button"
                onClick={restoreBookingTrackingModal}
                aria-label={`Mở lại trạng thái chuyến ${String(bookingSuccess.bookingCode ?? '').trim() || ''}`}
              >
                <span className="booking-tracking-bubble__icon-shell" aria-hidden="true">
                  <img className="booking-tracking-bubble__icon" src={motorbikeIcon} alt="" />
                </span>

                <span className="booking-tracking-bubble__copy">
                  <span className="booking-tracking-bubble__eyebrow">Chuyến của bạn</span>
                  <strong className="booking-tracking-bubble__title">
                    {String(bookingSuccess.tripStatusLabel ?? '').trim() || 'Xem lại trạng thái chuyến'}
                  </strong>
                  <span className="booking-tracking-bubble__subtitle">
                    {String(bookingSuccess.driverDisplayName ?? bookingSuccess.driverName ?? '').trim()
                      ? `Tài xế ${String(bookingSuccess.driverDisplayName ?? bookingSuccess.driverName).trim()}`
                      : 'Chạm để xem lại trạng thái chuyến'}
                  </span>
                </span>
              </button>,
              document.body,
        ...
        <DriverRideRequestModal
          ...
          request={activeDriverRideRequest?.request ?? null}
          distanceKm={activeDriverRideRequest?.distanceKm ?? null}
          isNearby={activeDriverRideRequest?.isNearby ?? true}
          disableKeyboardShortcuts={driverRideRejectModalOpen}
          onAccept={handleDriverRideRequestAccept}
          onReject={openDriverRideRejectModal}
        />

        <DriverRideRejectModal
          open={driverRideRejectModalOpen}
          request={activeDriverRideRequest?.request ?? null}
          onCancel={closeDriverRideRejectModal}
          onSubmit={handleDriverRideRequestRejectConfirm}
        />

        <DriverTripActionModal
          open={driverTripActionModalOpen}
          request={activeDriverTripAction}
          authenticatedAccountId={authenticatedUser?.id ?? ''}
          onClose={closeDriverTripActionModal}
          onMinimize={minimizeDriverTripActionModal}
          onAdvanceStage={handleDriverTripActionAdvance}
          onCancel={handleDriverTripActionCancel}
        />

        {driverTripActionBubbleOpen && activeDriverTripAction && !driverTripActionModalOpen
          ? createPortal(
              <button
                className="driver-trip-action-bubble"
                type="button"
                onClick={restoreDriverTripActionModal}
                aria-label={`Mở lại trạng thái chuyến ${String(activeDriverTripAction.bookingCode ?? '').trim() || ''}`}
              >
                <span className="driver-trip-action-bubble__icon-shell" aria-hidden="true">
                  <img className="driver-trip-action-bubble__icon" src={motorbikeIcon} alt="" />
                </span>

                <span className="driver-trip-action-bubble__copy">
                  <span className="driver-trip-action-bubble__eyebrow">Chuyến đang hoạt động</span>
                  <strong className="driver-trip-action-bubble__title">
                    {String(activeDriverTripAction.bookingCode ?? '').trim() ? `Mã chuyến ${String(activeDriverTripAction.bookingCode).trim()}` : 'Mở lại trạng thái chuyến'}
                  </strong>
                  <span className="driver-trip-action-bubble__subtitle">
                    {String(activeDriverTripAction.tripStatusLabel ?? '').trim() || 'Chạm để mở lại màn hình thao tác'}
                  </span>
                </span>
              </button>,
              document.body,
