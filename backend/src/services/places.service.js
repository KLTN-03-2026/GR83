...
function isPhotonGeocoderUnavailableError(error) {
  const message = String(error?.message ?? error ?? '');
  ...
  return (
    ...
    || name === 'AbortError'
    || name === 'TimeoutError'
    || /fetch failed|connect timeout|timed out|socket hang up|network/i.test(message)
  );
}

function isNominatimRateLimitedError(error) {
  const status = Number(error?.status ?? error?.cause?.status ?? NaN);
  const message = String(error?.message ?? error ?? '');

  return status === 429 || /HTTP\s+429|Too Many Requests/i.test(message);
}
...
async function fetchPhotonPredictions(query) {
  ...
  const response = await fetch(url, {
    headers: NOMINATIM_HEADERS,
    signal: AbortSignal.timeout(PHOTON_FETCH_TIMEOUT_MS),
  });
